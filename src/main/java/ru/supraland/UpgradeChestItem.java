...
public class UpgradeChestItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        // Создаём шарик-эффект
        spawnEffectBall(world, player);
    ...
    private void spawnEffectBall(World world, PlayerEntity player) {
        // Шарик частиц вокруг игрока
        for (int i = 0; i < 30; i++) {
        ...
        world.addParticle(ParticleTypes.TOTEM_OF_UNDYING,
            player.getX(), player.getY() + 1.0, player.getZ(), 0, 0.5, 0);
    }
}
